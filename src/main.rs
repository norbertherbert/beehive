...
use beehive::abw_ble_utils::{abw, find_dev::find_abw_device_names};
...
async fn main() -> Result<()> {
    ...
    let cli_arg_matches = command!() // requires `cargo` feature
        ...
        .subcommand(Command::new("scan").about("Scan for Abeeway devices."))
        .subcommand(
            Command::new("show").about("Show device details.").arg(
                arg!(
                    [device] "Device name."
                )
                .required(true),
        ...
        .subcommand(
            Command::new("remove-bond").about("Remove BLE bond.").arg(
                arg!(
                    [device] "Device name."
                )
                .required(true),
    ...
    let log_level = match cli_arg_matches
        .get_one::<u8>("verbose")
        .with_context(|| "couldn't get cli args")?
    {
    ...
    let ble_manager = Manager::new()
        .await
        .with_context(|| "cannot get BLE manager")?;
    let ble_adapters = ble_manager
        .adapters()
        .await
        .with_context(|| "cannot get BLE adapters")?;
    let ble_adapter = ble_adapters
        .into_iter()
        .nth(0)
    ...
    if let Some(_sub_cmd_matches) = cli_arg_matches.subcommand_matches("scan") {
        ...
        match found_abw_device_names.len() {
            ...
            1 => {
                println!(
                    "One Abeeway device was found:\n    {} - {}",
                    &found_abw_device_names[0].0,
                    if found_abw_device_names[0].1 {
                        "Connected"
                    } else {
                        "Not Connected"
                    }
    ...
    if let Some(sub_cmd_matches) = cli_arg_matches.subcommand_matches("import-config") {
        if let (Some(device_name), Some(file_path)) = (
            sub_cmd_matches.get_one::<String>("device"),
            sub_cmd_matches.get_one::<String>("file"),
        ) {
